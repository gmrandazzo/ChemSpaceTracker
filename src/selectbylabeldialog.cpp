...
int SelectByLabelDialog::getLabelID()
{
  return ui.comboBox->currentIndex();
}

SelectByLabelDialog::SelectByLabelDialog(QStringList labels): QDialog()
{
  ui.setupUi(this);
  for(int i = 0; i < labels.size(); i++){
    ui.comboBox->addItem(labels[i]);
  }
  connect(ui.selectButton, SIGNAL(clicked(bool)), SLOT(accept()));
